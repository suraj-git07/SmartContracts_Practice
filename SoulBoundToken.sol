...
// File: contracts/UniversityDegree.sol
...
contract UniversityDegree is ERC721URIStorage{
    ...
    function  issuedDegree(address _to) external OwnerOnly {
        issuedDegrees[_to] = true;
    }

    mapping(address=>string) public personToDegree; // anyone can see specific's address degree when claimed
    ...
    function claimDegree(string memory _tokenURI) public returns(uint){
        require(issuedDegrees[msg.sender] == true,"NO Degree is issued to you");

        tokenIds.increment(); //now not zero
        uint  newItemId = tokenIds.current();
        _mint(msg.sender,newItemId); // address , tokenId
        _setTokenURI(newItemId,_tokenURI);

        personToDegree[msg.sender] = _tokenURI;

        issuedDegrees[msg.sender] = false ;// not mint twice
